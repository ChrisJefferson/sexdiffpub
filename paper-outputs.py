...
def model1(*,priors, beliefDif,name ,allowLists,actParents,learnerAdditions):
    args = [(0.4, priors, beliefDif,allowLists,actParents,learnerAdditions)]
...
def model3cFunc(priors, beliefDif,allowLists,actParents,learnerAdditions):
    return runWorld(abilityDistribution = [0.2, 0.2, 0.6],
             abilityDistroUpdateTime = STEPS/10,
             abilityDistroUpdate = [0.0, 0.0, 1.0],
...
def model3c(*,priors, beliefDif,name ,allowLists,actParents,learnerAdditions):
    args = [(priors, beliefDif,allowLists,actParents,learnerAdditions)]

    with Pool(len(args)) as p:
        worlds = p.starmap(model3cFunc, args)
...
if __name__ == '__main__':
    ...
    dists = [ [0.0, 0.0, 1.0], [0.2, 0.2, 0.6], [0.2, 0.2, 0.6], [0.4, 0.4, 0.2], [0.4, 0.4, 0.2] ]
    
    priors = dists[priortype]

    beliefDif = [0,2,10,2,10][priortype]
